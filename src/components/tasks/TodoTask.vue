    ...
    <div class="modal__action" v-if="isShowModalAction">
      <div class="modal_links" v-click-outside="closeShowModalAction">
        <ul>
          ...
          <li><a href="javascript:;" @click.prevent="handleEditTask(dataModal.id)">Edit</a></li>
...
export default {
  ...
  methods:{
    ...
    handleSaveToStorage(status){
      ...
      this.todoTasks = this.allTasks.filter(items => items.categoryId == this.categoryId &&  items.status === status)
    },
    changeStatus(id , status){
      let idTask = this.allTasks.findIndex(item=> item.id === id)
      this.allTasks[idTask].status = status;
      this.handleSaveToStorage('todo')
    },
    handleDoingTask(id){
    ...
    handleEditTask(id){
      this.$router.push({name : 'EditTask' ,params:{slug : id}})
    },
    handleRemove(id){
  ...
  mounted() {
    ...
    if ('tasks' in localStorage){
      ...
      this.filterTasks = this.allTasks.filter(items => items.categoryId == this.categoryId &&  items.status === 'todo')
      let  n = 250;
      this.filterTasks.forEach((id, i)=>{
        setTimeout(()=>{
          this.todoTasks.push({...id})
        },n * (i + 1))
      })
    }
  }
